...
export default {
    ...
    data() {
        return {
            ...
            currentArea:
                this.determineType(this.area) || this.placeholders.area,
        };
    },
    created() {
        if (this.type !== "mobile") {
    ...
    watch: {
        ...
        currentCity(value) {
            this.$emit(
                "city",
                this.setData(value, "city", this.currentProvince)
            );
    ...
    methods: {
        ...
        determineValue(type, currentValue, placeholderValue, check = "") {
            if (currentValue === placeholderValue) {
                ...
                return this.getDistricts(
                    this.getAreaCode(currentValue, check, type)
                );
            }
        },
        determineType(value) {
        ...
        isDirectCity(province, city) {
            if (province && city) {
                return (
                    this.determineType(this.province) ===
                    this.determineType(this.city)
                );
            }
            return false;
